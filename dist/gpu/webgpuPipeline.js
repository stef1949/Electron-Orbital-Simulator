...
export function ensureWebGPUParticles(webgpu, n, l, m, numPoints) {
    ...
    const rSize = invCdf.image.width;
    const invT = ang.invThetaData;
    const tSize = ang.thetaSize;
    const invP = ang.invPhiData;
    const pSize = ang.phiSize;
    ...
    const invPBytes = tSize * pSize * 4 * BYTES;
    if (!webgpu.invPhiBuffer || webgpu._invPhiBytes !== invPBytes) {
        webgpu.invPhiBuffer?.destroy?.();
        webgpu.invPhiBuffer = device.createBuffer({ size: invPBytes, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
        webgpu._invPhiBytes = invPBytes;
    }

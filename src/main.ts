...
function hexToColor(hex: number) { const T = getTHREE(); return new T.Color(hex); }

window.addEventListener('load', async () => {
  const T: any = getTHREE();
  // --- Scene ---
  ...
  // UI Elements
  const buttons = document.querySelectorAll<HTMLButtonElement>('#orbital-panel .orbital-button');

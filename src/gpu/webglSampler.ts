...
export function createGPUSampleTarget(renderer: any, numPoints: number) {
  const T = getTHREE();
...
export function renderGPUSamples(renderer: any, caches: any, params: { n: number; l: number; m: number; numPoints: number; }) {
  const T = getTHREE();
...
export function createGPUPointsMesh(sampleTarget: any, THREERef: any, occlusionEnabled: boolean) {
  const T3 = getTHREE();

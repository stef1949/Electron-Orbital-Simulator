...
function assocLaguerre(p: number, alpha: number, x: number): number { if (p === 0) return 1; if (p === 1) return -x + alpha + 1; let Lkm2 = 1, Lkm1 = -x + alpha + 1; for (let k = 2; k <= p; k++) { const a = (2 * (k - 1) + alpha + 1 - x) * Lkm1; const b = ((k - 1) + alpha) * Lkm2; const Lk = (a - b) / k; Lkm2 = Lkm1; Lkm1 = Lk; } return Lkm1; }
function radialR(n: number, l: number, r: number): number { if (n <= l) return 0; const rho = 2 * r / n; const p = n - l - 1; const alpha = 2 * l + 1; const N = (2 / (n * n)) * Math.sqrt(fact(n - l - 1) / fact(n + l)); return N * Math.exp(-rho / 2) * Math.pow(rho, l) * assocLaguerre(p, alpha, rho); }
function assocLegendre(l: number, m: number, x: number): number { if (m < 0 || l < 0 || m > l) return 0; let pmm = 1; if (m > 0) { const somx2 = Math.sqrt(Math.max(0, 1 - x * x)); let odd = 1; for (let i = 1; i <= m; i++) odd *= (2 * i - 1); pmm = ((m % 2) ? -1 : 1) * odd * Math.pow(somx2, m); } if (l === m) return pmm; let pmmp1 = x * (2 * m + 1) * pmm; if (l === m + 1) return pmmp1; let pmmPrev = pmm, pmml = pmmp1, pll = 0; for (let L = m + 2; L <= l; L++) { pll = ((2 * L - 1) * x * pmml - (L + m - 1) * pmmPrev) / (L - m); pmmPrev = pmml; pmml = pll; } return pmml; }
function realY(l: number, m: number, theta: number, phi: number): number { const PI = Math.PI; const x = Math.cos(theta); const am = Math.abs(m); const Plm = assocLegendre(l, am, x); const norm = Math.sqrt((2 * l + 1) / (4 * PI) * fact(l - am) / Math.max(1, fact(l + am))); if (m === 0) return norm * Plm; const base = Math.SQRT2 * norm * Plm; return (m > 0) ? base * Math.cos(am * phi) : base * Math.sin(am * phi); }

// Caches
export const radialCache: Record<string, any> = {}; // key: `${n}_${l}` -> { radial: DataTexture, invCdf: DataTexture }
export const angularCache: Record<string, any> = {}; // key: `${l}_${m}` -> { invThetaTex, invPhiTex, thetaSize, phiSize, invThetaData, invPhiData }

export function createRadialLUT(n: number, l: number, size: number = 1024) {
...
export function createRadialInvCDF(n: number, l: number, size: number = 1024) {
  ...
  const T = getTHREE();
  const tex: any = new T.DataTexture(inv, size, 1, T.RGBAFormat, T.FloatType);
  tex.needsUpdate = true;
  tex.minFilter = T.LinearFilter;
  tex.magFilter = T.LinearFilter;
  tex.wrapS = T.ClampToEdgeWrapping;
...
export function createAngularInvTables(l: number, m: number, thetaSize: number = 256, phiSize: number = 256) {
...
export function getRadial(n: number, l: number, size: number = 1024) {
  const key = `${n}_${l}`;
...
export function getAngular(l: number, m: number, thetaSize: number = 256, phiSize: number = 256) {
  const key = `${l}_${m}`;

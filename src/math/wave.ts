...
function assocLaguerre(p: number, alpha: number, x: number): number {
  if (p === 0) return 1;
...
export function getWaveFunctionValue(n: number, l: number, m: number, r: number, theta: number, phi: number): number { return radialR(n, l, r) * realY(l, m, theta, phi); }

export function estimateMaxPsi2(n: number, l: number, m: number, samples: number = 1000): number {
  let maxPsi2 = 0;
...
export function estimateMaxAngular2(l: number, m: number, samples: number = 2000): number {
  let maxA2 = 0;
  for (let i = 0; i < samples; i++) {
    const u = Math.random();
    const v = Math.random();
